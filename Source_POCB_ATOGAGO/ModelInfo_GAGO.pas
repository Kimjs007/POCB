...
{$ENDIF}
    ...
    RzpnlPocbOptionYCamPosTitle   : TRzPanel;  // Model Information > Model Parameters > POCB Option > Ionizer On/Off //2019-09-26
    ...
    RzpnlPocbOptionPowerOnAgingSec: TRzPanel;
    edPocbOptionPowerOnAgingSec: TRzEdit;
    RzLblPocbOptionPowerOnAgingSec: TRzLabel;
    RzpnlPocbOptionCamCBCount: TRzPanel;
    cmbxPocbOptionCamCBCount: TRzComboBox;
    cbPocbOptionPowerResetAfterEepCBParaWrite: TRzCheckBox;
    ...
    RzPnlPocbOptionUniformityBase1: TRzPanel;
    ...
    RzPnlPocbOptionComparePat1: TRzPanel;
    cmbxPocbOptionVerifyCnt: TRzComboBox;
    RzPnlPocbOptionVerifyCount: TRzPanel;
    ...
    RzpnlPocbOptionBcrLen: TRzPanel;
    ...
    cbPocbOptionUseVacuum: TRzCheckBox;
    cbPocbOptionUseExLightFlow: TRzCheckBox;
    cbPocbOptionIonOnOff: TRzCheckBox;
...
begin
  ...
  if Trim(edModelName.Text) = '' then begin
    ...
    if (edPocbOptionYaxis1LoadPos.Value < Common.MotionInfo.YaxisSoftLimitMinus) or (edPocbOptionYaxis1CamPos.Value > Common.MotionInfo.YaxisSoftLimitPlus) then begin
    ShowMessage('Check YaxisSoftLimit' + CRLF + CRLF + 'MIN : ' + FormatFloat('0.00',Common.MotionInfo.YaxisSoftLimitMinus) + CRLF + 'MAX : ' + FormatFloat('0.00',Common.MotionInfo.YaxisSoftLimitPlus)
    + CRLF + CRLF + 'Input value CH1' + CRLF + CRLF+ 'Loading : ' + FormatFloat('0.00',edPocbOptionYaxis1LoadPos.Value) + CRLF + 'Camera : ' + FormatFloat('0.00',edPocbOptionYaxis1CamPos.Value));
    if edPocbOptionYaxis1LoadPos.Hint <> '' then edPocbOptionYaxis1LoadPos.Value := StrToFloatDef(edPocbOptionYaxis1LoadPos.Hint,0);
    if edPocbOptionYaxis1CamPos.Hint <> '' then edPocbOptionYaxis1CamPos.Value := StrToFloatDef(edPocbOptionYaxis1CamPos.Hint,0);
    Exit;

  end;

  if (edPocbOptionYaxis2LoadPos.Value < Common.MotionInfo.YaxisSoftLimitMinus) or (edPocbOptionYaxis2CamPos.Value > Common.MotionInfo.YaxisSoftLimitPlus) then begin
     ShowMessage('Check YaxisSoftLimit' + CRLF+ CRLF +'MIN : ' + FormatFloat('0.00',Common.MotionInfo.YaxisSoftLimitMinus) + CRLF + 'MAX : ' + FormatFloat('0.00',Common.MotionInfo.YaxisSoftLimitPlus)
   + CRLF + CRLF + 'Input value CH2' + CRLF + CRLF+ 'Loading : ' + FormatFloat('0.00',edPocbOptionYaxis2LoadPos.Value) + CRLF +'Camera : ' + FormatFloat('0.00',edPocbOptionYaxis2CamPos.Value));
    if edPocbOptionYaxis2LoadPos.Hint <> '' then edPocbOptionYaxis2LoadPos.Value := StrToFloatDef(edPocbOptionYaxis2LoadPos.Hint,0);
    if edPocbOptionYaxis2CamPos.Hint <> '' then edPocbOptionYaxis2CamPos.Value := StrToFloatDef(edPocbOptionYaxis2CamPos.Hint,0);
    Exit;

  end;          { TODO -oKimjs007 -c :  2024-02-14 오후 6:30:56 }
